...
# Extract and log URLs from curl commands
log_download_urls() {
    local command="$1"
    
    # Extract URLs from curl commands - handles both quoted and unquoted URLs
    # Pattern 1: curl -k -L "URL" or curl -k -LO "URL"
    # Pattern 2: curl -k -L URL or curl -k -LO URL
    local urls_quoted urls_unquoted
    
    # Extract quoted URLs: curl ... "https://..."
    urls_quoted=$(echo "$command" | grep -oE '"https?://[^"]*"' | tr -d '"' || true)
    
    # Extract unquoted URLs: curl ... https://... (but not inside quotes)
    urls_unquoted=$(echo "$command" | sed 's/"[^"]*"//g' | grep -oE 'https?://[^[:space:]&|;]*' || true)
    
    # Combine both results
    local all_urls
    all_urls=$(printf '%s\n%s\n' "$urls_quoted" "$urls_unquoted" | grep -v '^[[:space:]]*$' || true)
    
    if [ -n "$all_urls" ]; then
        while IFS= read -r url; do
            if [ -n "$url" ]; then
                log_progress "Download URL: $url"
            fi
        done <<< "$all_urls"
    fi
}
...
run_in_temp_dir() {
    ...
    # Log download URLs before executing command
    log_download_urls "$command"
