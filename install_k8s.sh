...
# ===============================================
# Installs kubectl, k9s, helm, helmfile, and configures themes.
# Uses version lock file to ensure consistent tool versions.
#
# Author: Alexis
# Version: 2.0
...
if [[ "${1:-}" == "--help" ]]; then
    ...
    echo " - helmfile   - Declarative spec for deploying helm charts"
    echo " - helm-diff  - Helm plugin for showing differences"
    exit 0
fi
...
if ! validate_dependencies "curl" "tar"; then
    error_exit "Missing critical dependencies for Kubernetes tools installation"
fi
...
echo " - helmfile:  v${HELMFILE_VERSION}"
...
# Install helm-diff with cleanup for incompatible versions
install_helm_diff() {
    if helm plugin list | grep -q "^diff"; then
        log_progress "Removing existing helm-diff plugin"
        helm plugin uninstall diff || true
    fi
    
    # Install latest compatible version
    helm plugin install https://github.com/databus23/helm-diff --version v3.9.11 || {
        log_warning "Failed to install specific version, trying without version constraint"
        helm plugin install https://github.com/databus23/helm-diff
    }
}

install_command "helm-diff" "install_helm_diff" "helm diff" "" true
...
echo "  k9s     - Kubernetes TUI with Catppuccin theme"
